...
let rectanglesDrawn = [];
let rotatingRectangles = new Set();
let pendingDeletion = [];
...
drawingZone.addEventListener('mousedown', (e) => {
    originX = e.clientX;
    originY = e.clientY;
...
drawingZone.addEventListener('mousemove', (e) => {
    if (!drawingPermission || !currentRectangle)
        return;
...
drawingZone.addEventListener('mouseup', () => {
    ...
    currentRectangle.addEventListener('dblclick', (event) => {
...
// Repaint the two rectangles with the least surface differences
